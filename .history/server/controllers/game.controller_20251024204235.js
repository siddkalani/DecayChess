...
export async function makeMove({ sessionId, userId, move, timestamp, variant, subvariant }) {
  ...
  // Don't clamp timers for classic variants - increment can make them exceed baseTime
  if (variant === "classic") {
    // Timer clamping is handled by the respective validators (blitz.js, bullet.js, standard.js)
  ...
  // Update timeControl.timers to match board timers for all variants
  if (variant === "classic" || variant === "decay") {
    gameState.timeControl = gameState.timeControl || {}
    gameState.timeControl.timers = {
      white: result.state.whiteTime,
      black: result.state.blackTime,
    }
  }
  ...
  // Note: For classic variants, timer increment is handled inside the respective validators
  // (blitz.js, bullet.js, standard.js) - no additional increment logic needed here
