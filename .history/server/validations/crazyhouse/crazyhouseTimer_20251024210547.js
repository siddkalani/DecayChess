...
function updateCrazyhouseTimers(state, currentTimestamp) {
  ...
  for (const color of ["white", "black"]) {
    ...
    if (color === currentPlayerColor) {
      // Find the first major piece (not pawn) in the pocket
      let firstMajorPieceIndex = -1
      for (let i = 0; i < pocket.length; i++) {
        if (pocket[i].type !== 'p') { // p = pawn
          firstMajorPieceIndex = i
          break
        }
      }

      // Only process timer if there's a major piece
      if (firstMajorPieceIndex !== -1) {
        const firstMajorPiece = pocket[firstMajorPieceIndex]
        const timerKey = firstMajorPiece.id
        ...
        if (currentTimestamp >= expirationTimestamp) {
          ...
          pocket.splice(firstMajorPieceIndex, 1)
          timers.delete(timerKey)
          state.frozenPieces[color].push(firstMajorPiece)
          console.log(`Major piece ${firstMajorPiece.type} expired for ${color}, moved to frozen.`)

          // Start timer for next major piece, if any
          let nextMajorPieceIndex = -1
          for (let i = 0; i < pocket.length; i++) {
            if (pocket[i].type !== 'p') { // p = pawn
              nextMajorPieceIndex = i
              break
            }
          }
          if (nextMajorPieceIndex !== -1) {
            const nextMajorPiece = pocket[nextMajorPieceIndex]
            timers.set(nextMajorPiece.id, currentTimestamp + DROP_TIME_LIMIT)
          }
        }
      }
    }
...
function handlePieceDrop(state, move, playerColor, game) {
  ...
  // Check if the piece to drop is a pawn (no timer restrictions)
  const isPawn = move.piece === 'p'
  
  if (isPawn) {
    // For pawns, find any pawn in the pocket (no sequential restriction)
    const pawnIndex = playerPocket.findIndex(piece => piece.type === 'p')
    if (pawnIndex === -1) {
      return { valid: false, reason: "No pawns in pocket", code: "PIECE_NOT_IN_POCKET" }
    }
    
    // Remove the pawn from pocket (no timer needed)
    playerPocket.splice(pawnIndex, 1)
    console.log(`Pawn dropped by ${playerColor} - no timer restrictions.`)
  } else {
    // For major pieces, use sequential drop with timer restrictions
    const firstMajorPieceIndex = playerPocket.findIndex(piece => piece.type !== 'p')
    if (firstMajorPieceIndex === -1) {
      return { valid: false, reason: "No major pieces in pocket", code: "PIECE_NOT_IN_POCKET" }
    }
    
    const firstMajorPiece = playerPocket[firstMajorPieceIndex]
    if (move.piece !== firstMajorPiece.type) {
      return {
        valid: false,
        reason: `Only ${firstMajorPiece.type} can be dropped next. Current queue: ${playerPocket.map((p) => p.type).join(", ")}`,
    ...
    if (!expirationTimestamp && firstMajorPiece.timerPaused && firstMajorPiece.remainingTime !== undefined) {
      expirationTimestamp = now + firstMajorPiece.remainingTime // Calculate effective expiration
    }

    // Check if piece has an active timer (either in map or paused on piece)
    if (!expirationTimestamp) {
      return { valid: false, reason: `No active timer found for piece ${firstMajorPiece.type}`, code: "PIECE_NOT_AVAILABLE" }
    }

    // Check if piece timer has expired
    if (now >= expirationTimestamp) {
      // Auto-remove expired piece
      playerPocket.splice(firstMajorPieceIndex, 1)
      playerDropTimers.delete(timerKey)
      delete firstMajorPiece.timerPaused // Clean up paused flag
      delete firstMajorPiece.remainingTime // Clean up remaining time

      // Start timer for next major piece, if any
      const nextMajorPieceIndex = playerPocket.findIndex(piece => piece.type !== 'p')
      if (nextMajorPieceIndex !== -1) {
        const nextMajorPiece = playerPocket[nextMajorPieceIndex]
        playerDropTimers.set(nextMajorPiece.id, now + DROP_TIME_LIMIT)
        console.log(`Started timer for next major piece ${nextMajorPiece.type} after expired drop attempt.`)
      }

      console.warn(`Attempted to drop expired major piece: ${firstMajorPiece.type}. Removed from pocket.`)
      // Return a warning, do not stop the game
      return {
        valid: false,
        type: "game:warning",
        reason: `Major piece ${firstMajorPiece.type} drop limit expired`,
        code: "DROP_EXPIRED",
        gameEnded: false
      }
    }
  ...
  const pieceType = isPawn ? 'p' : firstMajorPiece.type
  ...
  try {
    testGame.put({ type: pieceType, color: playerColor === "white" ? "w" : "b" }, move.to)
  ...
  try {
    game.put({ type: pieceType, color: playerColor === "white" ? "w" : "b" }, move.to)
    ...
    // Remove from pocket and timer (already handled above for pawns)
    if (!isPawn) {
      // For major pieces, remove from pocket and timer
      playerPocket.splice(firstMajorPieceIndex, 1)
      playerDropTimers.delete(timerKey)
      delete firstMajorPiece.timerPaused // Clean up paused flag
      delete firstMajorPiece.remainingTime // Clean up remaining time
      ...
      if (nextMajorPieceIndex !== -1) {
        const nextMajorPiece = playerPocket[nextMajorPieceIndex]
        playerDropTimers.set(nextMajorPiece.id, now + DROP_TIME_LIMIT)
        console.log(`Started timer for next major piece ${nextMajorPiece.type} after successful drop.`)
      }
    }
    ...
    return {
      ...
      result: {
        ...
        to: move.to,
        from: "pocket",
        san: `${pieceType.toUpperCase()}@${move.to}`,
