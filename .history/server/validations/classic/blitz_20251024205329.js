...
// Create initial state for a blitz game (3+2)
export function createInitialState() {
  try {
    ...
    const now = Date.now()
    const blitzTime = 3 * 60 * 1000 // 3 minutes in milliseconds
    const increment = 2 * 1000 // 2 seconds increment in milliseconds
    ...
    return {
      ...
      whiteTime: blitzTime,
      blackTime: blitzTime,
      increment: increment, // 2 second increment per move
      timeControl: "blitz", // Identify game type
    ...
    console.error("Error creating initial blitz state:", error)
    throw error
  }
}

// Validate a move and update timers with increment
export function validateAndApplyMove(state, move, playerColor, currentTimestamp) {
  try {
    console.log("=== BLITZ MOVE VALIDATION START ===")
    ...
    console.log("Increment:", state.increment, "ms")
    ...
    // Initialize timer values and increment if missing
    ...
    if (typeof state.whiteTime !== "number") state.whiteTime = 3 * 60 * 1000
    if (typeof state.blackTime !== "number") state.blackTime = 3 * 60 * 1000
    if (typeof state.increment !== "number") state.increment = 2 * 1000
    ...
    // BLITZ SPECIFIC: Add increment to the player who just moved (3+2 time control), clamp to 3 min
    // Copy exact logic from decay.js but for blitz (3+2 time control)
    const BASE_TIME = 180000 // 3 minutes in ms
    if (currentPlayerBeforeMove === "w") {
      state.whiteTime = Math.min(state.whiteTime + state.increment, BASE_TIME)
      console.log(`WHITE MOVE: Adding ${state.increment}ms increment. New white time: ${state.whiteTime}`)
    } else {
      state.blackTime = Math.min(state.blackTime + state.increment, BASE_TIME)
      console.log(`BLACK MOVE: Adding ${state.increment}ms increment. New black time: ${state.blackTime}`)
    }
    ...
    console.log("- Final times after increment - White:", state.whiteTime, "Black:", state.blackTime)
    ...
    console.log("=== BLITZ MOVE VALIDATION END ===")
    ...
    console.error("Error in blitz validateAndApplyMove:", error)
    return {
      valid: false,
...
// Get current timer values for blitz games
export function getCurrentTimers(state, currentTimestamp) {
  try {
    // Validate input
    if (!state || typeof state !== "object") {
      console.error("[BLITZ TIMER] Invalid state provided to getCurrentTimers")
      return {
        white: 3 * 60 * 1000,
        black: 3 * 60 * 1000,
        activeColor: "white",
    ...
    if (state.gameEnded) {
      return {
        ...
        endTimestamp: state.endTimestamp,
        timeControl: "blitz",
        increment: state.increment || 2 * 1000,
      }
    }

    if (!state.gameStarted || !state.turnStartTimestamp || state.moveHistory.length === 0) {
      return {
        white: state.whiteTime || 3 * 60 * 1000,
        black: state.blackTime || 3 * 60 * 1000,
        activeColor: state.activeColor || "white",
        gameEnded: false,
        timeControl: "blitz",
        increment: state.increment || 2 * 1000,
    ...
    try {
      ...
      console.error("[BLITZ TIMER] Error reconstructing game from FEN:", error)
      return {
        white: state.whiteTime || 3 * 60 * 1000,
        black: state.blackTime || 3 * 60 * 1000,
        activeColor: state.activeColor || "white",
    ...
    let whiteTime = state.whiteTime || 3 * 60 * 1000
    let blackTime = state.blackTime || 3 * 60 * 1000
    ...
    console.error("Error in blitz getCurrentTimers:", error)
    return {
      white: state?.whiteTime || 3 * 60 * 1000,
      black: state?.blackTime || 3 * 60 * 1000,
      activeColor: state?.activeColor || "white",
...
export function getLegalMoves(fen) {
  try {
    if (!fen || typeof fen !== "string") {
      console.error("[BLITZ MOVES] Invalid FEN provided to getLegalMoves:", fen)
      return []
    }
...
export function checkGameStatus(state, gameInstance) {
  try {
    // Validate input
    if (!state || typeof state !== "object") {
      console.error("[BLITZ STATUS] Invalid state provided to checkGameStatus")
    ...
    if (!game) {
      if (!state.fen) {
        console.error("[BLITZ STATUS] Missing FEN in game state")
        return { result: "ongoing", error: "Missing FEN" }
      }
      try {
        game = new Chess(state.fen)
      } catch (error) {
        console.error("[BLITZ STATUS] Error reconstructing game from FEN:", error)
    ...
    if (game.isCheckmate()) {
      ...
      console.log(`BLITZ CHECKMATE DETECTED: ${winnerColor} wins!`)
    ...
    console.error("Error checking blitz game status:", error)
...
export function updateRepetitionMap(state, gameInstance) {
  try {
    // Validate input
    if (!state || typeof state !== "object") {
      console.error("[BLITZ REPETITION] Invalid state provided to updateRepetitionMap")
      return
    }
    ...
    if (gameInstance) {
      ...
      console.error("[BLITZ REPETITION] Missing FEN in game state")
      return
    }

    if (!fen || typeof fen !== "string") {
      console.error("[BLITZ REPETITION] Invalid FEN format:", fen)
      return
    }
    ...
    console.log("Blitz repetition map updated for FEN:", fen, "Count:", current + 1)
  } catch (error) {
    console.error("Error updating blitz repetition map:", error)
  }
}